...
if (!text) throw `*[❗] هذا لامر خاص بتحميل من منصة X (تويتر), مثال: ${usedPrefix + command}* https://twitter.com/auronplay/status/1586487664274206720?s=20&t=3snvkvwGUIez5iWYQAehpw`;
if (enviando) return;
    enviando = true;
try {
 ...
 if (res?.result.type == 'video') {
     const caption = res?.result.caption ? res.result.caption : '*هاهية صورتك*';
...
handler.command = /^((x|xdl|تويت|twdl|tw|twt|twitter)(dl)?)$/i;
