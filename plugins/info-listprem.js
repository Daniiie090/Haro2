...
—◉ المستخدم: ${userr}
${prem ? `*◉ الوقت:*\n${clockString(usuario - new Date() * 1)}` : (isPrems ? `*◉ وقت:*\n-  هو مستخدم متميز لفترة غير محدودة` : '- هذا هو مستخدم بدون قسط ❌')}

*「 المستخدين المميزين 」*${sortedP.slice(0, len).map(({jid, name, premiumTime, prem, registered}, i) => `

—◉ الاسم: ${'@' + jid.split`@`[0]}
${premiumTime > 0 ? `*◉ الوقت:*\n${clockString(premiumTime - new Date() * 1)}` : '-هذا هو مستخدم بدون قسط❌'}`).join('')}`.trim();

  if (sortedP.filter((user) => user.premiumTime).length === 0) {
    infoprem = `*「 هل انت مستخدم مميز؟ 」*\n\n—◉ المستخدم: ${userr}\n${prem ? `*◉ الوقت المتبقي:*\n${clockString(usuario - new Date() * 1)}` : '- أنت لست مستخدمًا متميزًا ❌'}\n\n*「 المستخدمين المميزين 」*\n\n- لا يوجد مستخدمين متميزين❌`.trim();
  }
