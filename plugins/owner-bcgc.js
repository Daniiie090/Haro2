const handler = async (m, {conn, isROwner, text}) => {
  ...
  if (!pesan) throw '*[❗تنبيه❗] أدخل الرسالة التي تريد نشرها*';
  ...
  for (const i of anu) {
    ...
    conn.relayMessage(i,
        {liveLocationMessage: {
          ...
          caption: '[❗تنبيه هام❗]\n\n' + pesan + '\n\nهذا بيان رسمي من المطور',
          sequenceNumber: 2,
          timeOffset: 3,
          contextInfo: m,
        }}, {}).catch((_) => _);
  }
  m.reply(`*[❗تنبيه❗] تم الارسال ${anu.length} المجموعة*\n\n*ملاحظة:  من الممكن أن يكون هذا الأمر به أخطاء ولا يتم إرساله لجميع الدردشات، آسف على التأخير.*`);
};
handler.help = ['broadcastgroup', 'bcgc'].map((v) => v + ' <teks>');
handler.tags = ['owner'];
handler.command = /^(غرو|bc)(group|grup|gc)$/i;
handler.owner = true;
