...
const handler = async (m, {conn, text, args, usedPrefix, command}) => {
  if (!text) throw `*[❗تنبيه❗] هذا لامر خاص بتحميل مقاطع التيك توك*\n\n*—◉ مثال:*\n*${usedPrefix + command}* https://vm.tiktok.com/ZMYWFhKt2/`;
  if (!/(?:https:?\/{2})?(?:w{3}|vm|vt|t)?\.?tiktok.com\/([^\s&]+)/gi.test(text)) throw `*[❗𝐈𝐍𝐅𝐎❗] هذا لامر خاص بتحميل مقاطع التيك توك*\n\n*—◉ مثال:*\n*${usedPrefix + command}* https://vm.tiktok.com/ZMFb4BXVd/`;
  const texto = `*[❗] @${m.sender.split`@`[0]} جاري التحميل سيأخذ وقتا*`;
  ...
  try {
    ...
    const desc1 = `*¡مرحبا! 🌟*\n*📽️ هاهو المقطع الذي طلبته.*\n\n* اذا اردت تحويله لصوت تذكر انه يمكنك طلب لامر tm/ta🎧.*ا`;
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          throw `*[❗تنبيه❗] حدث خطا اثناء التحميل ربما المقطع فيه خلل*`;
        }
      }
    }
  }
};
handler.command = /^(tt|تيك|tiktokdl|tiktoknowm|tt|توك|tiktokaudio)$/i;
export default handler;

async function tiktokdlF(url) {
  if (!/tt/.test(url)) return 'Enlace incorrecto';
