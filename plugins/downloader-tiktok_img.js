...
if (!tiktok) throw '*[❗] هذا لامر خاص بتحميل صور من التيك توك مثال: "https://vm.tiktok.com/ZM2cqBRVS/".*';        
let imagesSent
...
imagesSent = false
    ...
    imagesSent = false    
    throw '*[❗] لاتوجد استجابة من صفحة التيك توك حاول لاحقا.*'
 }
};
handler.command = /^(tif|تيف)$/i;
...
async function ttimg(link) {
    try {    
        ...
        if (imgSrc.length === 0) {
            return { data: '*[❗] لم يتم لعثور على صور من الرابط.*' };
        }
        return { data: imgSrc }; 
    } catch (error) {
        console.lo (error);
