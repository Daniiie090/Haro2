...
const handler = async (m, {conn, args, command, usedPrefix}) => {
  if (!args[0]) throw `*[❗𝐈𝐍𝐅𝐎❗] ارسل رابط المقطع الذي تريد تحميله من الفيس بوك مثال: ${usedPrefix + command}* https://fb.watch/fOTpgn6UFQ/`;
  if (!args[0].match(/www.facebook.com|fb.watch/g)) throw `*[❗𝐈𝐍𝐅𝐎❗] ارسل رابط المقطع الذي تريد تحميله من الفيس بوك: ${usedPrefix + command}* https://fb.watch/fOTpgn6UFQ/`;
  try {
    await m.reply(`*[❗]جاري تنزيل ل الخاص بك يرجى الانتظار قد يستغرق التحميل من دقيقتين الى عشر دقائق حسب مدة الفيديو...*`);
    const d2ata = await facebook.v1(args[0]);
    let r2es = '';
    if (d2ata.urls && d2ata.urls.length > 0) {
      r2es = `${d2ata.urls[0]?.hd || d2ata.urls[1]?.sd || ''}`;
    }
    conn.sendFile(m.chat, r2es, 'error.mp4', `*حدث خطأ اثناء تحميله*`, m);
  } catch (err1) {
    try {
          ...
          try {
            ...
            try {
              ...
              try {
                ...
                throw `*[❗𝐈𝐍𝐅𝐎❗] حدث خطأ اثناء التحميل*`;
              }
            }
          }
        }
