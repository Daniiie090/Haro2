...
const handler = async (m, {conn, text, usedPrefix, command}) => {
  if (!text) throw `*[❗𝐈𝐍𝐅𝐎❗] ابحث عن محتوى تريده صورا مثل${usedPrefix + command} Minecraft*`;
  ...
  const link = image;
  conn.sendFile(m.chat, link, 'error.jpg', `🔎 *هاهو:* ${text}\n🔗 *𝙻𝙸𝙽𝙺* ${link}\n🌎 *𝙱𝚄𝚂𝙲𝙰𝙳𝙾𝚁:* Google`, m);
};
handler.help = ['gimage <query>', 'imagen <query>'];
handler.tags = ['internet', 'tools'];
handler.command = /^(gimage|image|صور)$/i;
