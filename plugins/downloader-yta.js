...
const handler = async (m, {text, conn, args, usedPrefix, command}) => {
  if (!args[0]) throw '*[❗] هذا لامر خاص بتحميل من يوتيوب عن طريق رابط.*';  
  let enviando;
  if (enviando) return  
      enviando = true      
  let youtubeLink = '';
      ...
      if (Array.isArray(global.videoList) && global.videoList.length > 0) {
        ...
        if (matchingItem) {
          if (index < matchingItem.urls.length) {
            ...
            throw `*[❗] الرابط خاطئ ${matchingItem.urls.length}*`;
          }
        } else {
  ...
  const { key } = await conn.sendMessage(m.chat, {text: `*_⏳جاري التحميل...⏳_*\n\n*◉ يمكنك التحميل ايضا من خلال #يوتب ᴏ #play.2 ᴏ #yt2 ◉*`}, {quoted: m});
  try {
