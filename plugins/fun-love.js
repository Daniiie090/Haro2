const handler = async (m, { conn, command, text }) => {
  ...
  if (m.message.conversation === "حبي") {
    const emoji = "👍"; // Define the emoji you want the bot to react with
    try {
  ...
  const response =
    `━━━━━━━⬣ *LOVE* ⬣━━━━━━━\n` +
    `*❥ قياس نسبة الحب, ${text} y @${m.sender.split('@')[0]} ${loveDescription} del ${lovePercentage}% de un 100%*\n\n` +
    `*❥ ${loveMessage}*\n` +
    `━━━━━━━⬣ *LOVE* ⬣━━━━━━━`    
  
  async function loading() {
var hawemod = [
"《 █▒▒▒▒▒▒▒▒▒▒▒》10%                        جاري قياس نسبة حبك       ",
"《 ████▒▒▒▒▒▒▒▒》30%                         معرفة التشابه       ",
"《 ███████▒▒▒▒▒》50%                       استكشاف  الصفات  ",
"《 ██████████▒▒》80%                      جاري تخزين المعلومات     ",
"《 ████████████》100%                      جاري التقديم النهائي           "
]
   let { key } = await conn.sendMessage(m.chat, {text: `*💞 ¡نسبة حبك هية! 💞*`, mentions: conn.parseMention(response)}, {quoted: m})
