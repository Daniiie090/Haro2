...
const handler = async (m, {conn, args, usedPrefix, command}) => {
  if (!args[0]) throw '*[❗ رجاء ادخل رابط المقطع الذي تريد تحميله.*';
  let enviando;
  if (enviando) return  
      enviando = true    
  let youtubeLink = '';
  ...
  const { key } = await m.reply(`*_⏳جاري التحميل  الان...⏳_*\n\n*◉ يمكنك التحميل من هنا#يوتب ᴏ #play.2 ᴏ #yt2◉*`);
  try {
  ...
  try {
    ...
    try {
      ...
      await conn.sendMessage(m.chat, {video: {url: mediaa.result}, fileName: `error.mp4`, caption: `_GoJo_Bots`, thumbnail: mediaa.thumb, mimetype: 'video/mp4'}, {quoted: m});
      await conn.sendMessage(m.chat, {text: '*[ ✔ ] اكتمل تحميل الفيديو.*', edit: key}, {quoted: m});
      enviando = false
    } catch {
      try {
...
handler.command = /^(video|ytv|dlmp4|getvid|yt(v|mp4)?)$/i;
