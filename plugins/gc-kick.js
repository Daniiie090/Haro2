const handler = async (m, {conn, participants, command, usedPrefix}) => {
  if (!global.db.data.settings[conn.user.jid].restrict) throw '*[ ⚠️ ] على المطور ان يقوم بتفعيل الطرد*';
  const kicktext = `*[❗] قم بمنشنة الشخص مع لامر*\n\n*—◉ مثال:*\n*${usedPrefix + command} @${global.suittag}*`;
...
handler.help = ['kick'];
handler.tags = ['group'];
handler.command = /^(kick|echar|hechar|انذار)$/i;
